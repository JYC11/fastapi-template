...
async def exception_handler(request: Request, call_next):
    ...
    except service_exceptions.ItemNotFound as e:
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content={"error": "Not found", "message": str(e)},
        )
    except service_exceptions.MethodNotFound as e:
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"error": "Incorrect method call in message bus", "message": str(e)},
        )
    except service_exceptions.Unauthorized as e:
        return JSONResponse(
            status_code=status.HTTP_401_UNAUTHORIZED,
            content={"error": "Unauthorized", "message": str(e)},
        )
    except service_exceptions.Forbidden as e:
        return JSONResponse(
            status_code=status.HTTP_403_FORBIDDEN,
            content={"error": "Forbidden", "message": str(e)},
        )
    except service_exceptions.DuplicateRecord as e:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"error": "Duplicate record exists", "message": str(e)},
        )
    except service_exceptions.ConcurrencyException as e:
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
