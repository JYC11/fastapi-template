...
async def create_test_user(app_for_test: FastAPI):
    user_data_in = {
        "phone": "010",
    ...
    async with AsyncClient(
        app=ASGITransport(app=app_for_test),  # type: ignore
...
async def test_user_login(app_for_test: FastAPI):
    login_data = {
        "username": "email@email.com",
        "password": "password",
    }

    login_url = app_for_test.url_path_for("login")
    async with AsyncClient(
        app=ASGITransport(app=app_for_test),  # type: ignore
        base_url=settings.test_url,
    ) as ac:
...
async def create_user_and_login(app_for_test: FastAPI):
